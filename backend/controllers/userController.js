...
const getUserProfile = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to retrieve your profile.' });
  }
};
...
const getUserLibrary = async (req, res) => {
  try {
    const orders = await prisma.order.findMany({
      ...
      select: {
        product: { select: { id: true, title: true, thumbnail: true, category: true } },
      },
    ...
    res.status(500).json({ error: 'Failed to synchronize your library.' });
  }
};
...
const downloadAsset = async (req, res) => {
  try {
    ...
    const order = await prisma.order.findFirst({
      where: { userId: req.user.id, productId, status: 'SUCCESS' },
    ...
    res.status(500).json({ error: 'Failed to prepare the secure download.' });
  }
};
