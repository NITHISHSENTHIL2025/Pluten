...
type Customer={id:string;email:string;firstName:string|null;lastName:string|null;isPremium:boolean;createdAt:string;_count?:{orders:number;portfolios:number}};
...
export default function AdminCustomersPage(){
  ...
  return <main className={styles.dashboardContainer}>
    ...
    {loading?<div className={styles.authLoading}><Loader2 size={28} className="pluten-login-spinner"/><span>Loading customers</span></div>:error?<div className={styles.errorState}><AlertCircle size={22}/><h2>Customer directory unavailable.</h2><p>{error}</p><button className={styles.primaryButton} onClick={()=>load(pagination.page)}>Retry</button></div>:rows.length===0?<div className={styles.emptyState}><Users size={24}/><h2>No customers found.</h2><p>Try a different search.</p></div>:<>
      <div className={styles.tableCard}><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th className={styles.th}>Customer</th><th className={styles.th}>Email</th><th className={styles.th}>Purchases</th><th className={styles.th}>Portfolios</th><th className={styles.th}>Status</th><th className={styles.th}>Joined</th></tr></thead><tbody>{rows.map(c=><tr key={c.id}><td className={styles.td}><strong>{`${c.firstName||''} ${c.lastName||''}`.trim()||'Unnamed customer'}</strong></td><td className={styles.td}>{c.email}</td><td className={styles.td}><Link href={`/admin/customers/${c.id}`}>{c._count?.orders||0}</Link></td><td className={styles.td}>{c._count?.portfolios||0}</td><td className={styles.td}>{c.isPremium?'Premium':'Standard'}</td><td className={styles.td}>{formatIndiaDate(c.createdAt)}</td></tr>)}</tbody></table></div></div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginTop:16,color:'var(--pluten-text-secondary)',fontSize:11}}><span>Page {pagination.page} of {pagination.totalPages}</span><div style={{display:'flex',gap:6}}><button className={styles.iconBtn} disabled={pagination.page<=1} onClick={()=>load(pagination.page-1)}><ChevronLeft size={16}/></button><button className={styles.iconBtn} disabled={pagination.page>=pagination.totalPages} onClick={()=>load(pagination.page+1)}><ChevronRight size={16}/></button></div></div>
    </>}
  </main>;
}
