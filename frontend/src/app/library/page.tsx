...
import { ArrowLeft, Download, Library, ShieldCheck, UserRound, AlertCircle, RefreshCw } from "lucide-react";
...
export default function MyLibraryPage(){
  ...
  const loadLibrary=async()=>{
    ...
    try{const response=await apiClient.get<PurchasedAsset[]>("/user/library", { skipSessionExpiry: true, skipApiErrorLog: true });setAssets(Array.isArray(response.data)?response.data:[]);}
      ...
      if(err?.response?.status===401||err?.response?.status===403){router.replace(`/login?expired=1&redirect=${encodeURIComponent("/library")}`);return;}
  ...
  if(loading)return <main className={styles.pageContainer}><div className={styles.loader}><div className={styles.loadingContent}><PlutenMotion state="loading" size={78} className={styles.loadingLogo} label="Opening your library"/><span className={styles.loadingText}>Opening your library</span></div></div></main>;
  ...
  return <main className={styles.pageContainer}>
    ...
    <div className={styles.contentWrapper}>
      ...
      {error?<section className={styles.errorPanel}><PlutenMotion state="error" size={72} label="Library unavailable"/><h2 className={styles.errorTitle}>Library unavailable.</h2><p className={styles.errorText}>{error}</p><button className={styles.retryBtn} onClick={loadLibrary}><RefreshCw size={15}/> Try again</button></section>:<>
